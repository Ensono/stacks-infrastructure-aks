...
| `setup:validate:azdo:pat` | Validate that the Azure DevOps Personal Access Token has the minimum required scopes (Project and Team: Read, Variable Groups: Read & manage). Fails fast if scopes are missing. Skipped if `TF_VAR_create_ado_variable_group=false`.
...
| `tests:infra:inputs` | Generate the Inspec inputs file from Terraform outputs and Azure version data
...
The tasks are defined in `build/eirctl/tasks.yaml`

==== Task Dependency Graph

[graphviz]
----
include::diagrams/task-dependency-graph.dot[]
----

==== Validation Placement

The PAT validation task (`setup:validate:azdo:pat`) runs immediately after `setup:environment` and before any Terraform initialization.
...
Terraform outputs are converted to `TFOUT_` variables using `Set-EnvironmentVars`, and these are used by the Helm deployment and InSpec tests.
...
| Upgrade Providers | `upgrade_providers` | `false` | Runs `terraform init -upgrade` to update provider versions to match constraints in provider.tf. Use when upgrading major provider versions (e.g., AzureRM 3.x → 4.x).
| Stages | `stages` | | Parameterized array defining deployment stages with environment-specific configuration (DNS zones, resource naming, security policies).

Each stage includes `environment_shortname` (maps to `TF_VAR_stage`), `is_prod_subscription` (controls security posture), and deployment conditions.

See xref:azure-devops-integration.adoc#_stage_configuration_structure[Stage Configuration] for customization options.
|===
...
----
az pipelines run \
    --name "amido.stacks-infrastructure-aks (1)" \
