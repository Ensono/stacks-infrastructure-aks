...
.Pipeline parameters
[cols="1,1,1,4a",options="header",stripes=even]
|===
| Parameter | ID | Default | Description
| Deploy Environment | `deploy` | `true` | Deploy the environment
...
| Stages | `stages` |  a| Parameterized array defining deployment stages with environment-specific configuration (DNS zones, resource naming, security policies).
