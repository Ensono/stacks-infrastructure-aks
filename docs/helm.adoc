...
[,yaml]
----
include::../deploy/helm/k8s_apps.yaml[]
----

This file is read by the `Deploy-Helm` cmdlet in the Independent Runner PowerShell module. The cmdlet can handle installing from a remote chart repository or from a local one contained on the file system.
...
----
include::../deploy/helm/values/cluster_setup.yaml[]
----
...
=== Terraform → Helm Data Flow

[graphviz]
----
include::diagrams/terraform-helm-dataflow.dot[]
----

=== External DNS Configuration

External DNS is a critical component for automatic DNS management in AKS:

**Purpose**:

* Automatically creates DNS records for Kubernetes Ingress resources
* Synchronizes Kubernetes service discovery with Azure DNS
* Supports both public (Azure DNS) and private (Private DNS) zones
* Enables declarative DNS management without manual zone updates

**Configuration**:

The External DNS Helm chart uses:

* `TFOUT_aks_identity_client_id` – Managed identity for DNS permissions
* `TFOUT_dns_zone_name` – Target DNS zone for records
* `TFOUT_resource_group_name` – Resource group containing DNS zone

**Example Values Template** (`deploy/helm/values/external_dns.yaml`):

[source,yaml]
----
azure:
  resourceGroup: "{{ env "TFOUT_resource_group_name" }}"
  clientId: "{{ env "TFOUT_aks_identity_client_id" }}"
...
. Ingress resource created with hostname
. External DNS controller detects change
. Checks Azure DNS zone for permissions
. Creates or updates A/CNAME record
. DNS propagates to nameservers

=== Worked Example

The Helm deployment uses Terraform outputs exposed as environment variables:

[source,bash]
----
TFOUT_aks_cluster_name=aks-ens-stacks-uks-nonprod
TFOUT_resource_group_name=rg-ens-stacks-uks-nonprod
TFOUT_app_gateway_ip=198.51.100.10
----

These values are injected into the chart values template during `infra:helm:apply`.
