...
Edit this file as needed and then run the script `. ./local/envvar-azure-stacks-aks.ps1`. This will then set up the necessary environment variables in your local shell. These will then be copied into the container when it is spun up by eirctl.
...
The resultant script will be created in `local/envvar-azure-dev.sh`. The naming convention is `envvar-<CLOUD>-<STAGE>.sh`.
...
[source,bash]
----
source ./local/envvar-azure-dev.sh
----
...
. Environment script (`local/envvar-azure-<stage>.*`)
