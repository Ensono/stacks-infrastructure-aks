...
[source,yaml]
----
TF_VAR_aks_node_pools="{\"np1\":{\"vm_size\":\"Standard_D4s_v5\",\"auto_scaling\":true,\"min_nodes\":1,\"max_nodes\":3,\"enable_availability_zones\":false,\"availabilty_zones\":[]}}"
----

The `enable_availability_zones` and `availabilty_zones` fields match the upstream `azurerm-aks` module contract. The `availabilty_zones` spelling is intentional and must not be corrected to `availability_zones`.
...
The AKS deployment uses Ensono Stacks modules from the link:https://github.com/Ensono/stacks-terraform[stacks-terraform repository]. Module versions are specified in link:../deploy/terraform/aks.tf[deploy/terraform/aks.tf] and link:../deploy/terraform/ssl_app_gateway.tf[deploy/terraform/ssl_app_gateway.tf]. These provide:
