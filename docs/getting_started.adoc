...
| `TF_VAR_stage` | Deployment stage / target environment (e.g., dev, nonprod, prod) | [green]#icon:check[]#
...
| `TF_VAR_internal_ingress_enabled` | Selects the internal-ingress topology used by the Application Gateway integration. Must be `true` when `TF_VAR_create_ssl_gateway=true` in this stack. | [green]#icon:check[]#
...
NOTE: In Azure DevOps pipelines, environment configuration is controlled by the `stages` parameter object, which maps `environment_shortname` to `TF_VAR_stage` and controls deployment behavior via `is_prod_subscription`. See xref:azure-devops-integration.adoc#_stage_configuration_structure[Stage Configuration] for detailed pipeline configuration options.
