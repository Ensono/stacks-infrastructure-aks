locals {
  ...
  environments_all = {
    for env_parts in [
      for env_definition in split(",", var.environment_definitions) :
      split(":", env_definition)
    ] :
    env_parts[0] => {
      is_prod = env_parts[1] == "true" ? true : false
    }
  }
  ...
  outputs = { for envname in local.environments : envname => merge(local.resource_outputs[envname], {
    ...
    certificate_pem                  = var.create_ssl_gateway ? module.ssl_app_gateway[0].certificate_pem : null
    ...
    issuer_pem                       = var.create_ssl_gateway ? module.ssl_app_gateway[0].issuer_pem : null
