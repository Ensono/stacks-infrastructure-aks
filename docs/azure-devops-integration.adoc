...
. The task verifies read permissions, attempts a create/delete test variable group, and calls the Azure DevOps protected resource authorization endpoint used by the Terraform provider.
...
* Pipeline resources: Use and manage (`vso.pipelineresources_manage`) – required because `azuredevops_variable_group` also calls the `authorizedresources` API to synchronize project/pipeline access for the variable group
...
* Missing scopes
* Expired token
* PAT can create variable groups but cannot authorize them for project/pipeline resource use
...
* `is_prod_subscription: false` - marks subscription as non-production for environment selection and tagging
...
* `is_prod_subscription: true` - marks subscription as production for environment selection and tagging
* Requires all previous stages to succeed
* Production-level compliance and security checks (via separate Azure Policy assignments and role definitions)
...
The `is_prod_subscription` boolean is passed from the Azure DevOps stage into Terraform and is used to control how environments are selected and tagged.

In the current implementation:

* Terraform uses `is_prod_subscription` to determine which environments from `environment_definitions` are deployed for a given run (for example, selecting production-only environments when `true`).
* It can be used to drive subscription-level metadata or tagging that distinguishes production from non-production subscriptions.
* While production controls like Azure Policy enforcement, stricter RBAC, audit logging, and SKU selection are important, they are separate concerns and must be configured independently (via Azure Policy assignments, role definitions, or pipeline protection rules).

Set `is_prod_subscription: true` only for production subscriptions so that Terraform applies the correct environment selection and subscription tagging behavior.
